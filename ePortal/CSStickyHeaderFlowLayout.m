...
- (void)prepareLayout {
  ...
  if (self.enableDecorationView) {
    [self registerClass:[CSSCollectionSeparator class] forDecorationViewOfKind:CSSDecorationKind];
  }
}
...
//     [self debugLayoutAttributes:allItems];
    ...
    if (self.enableDecorationView) {
      ...
      for (NSIndexPath *indexPath in visibleIndexPaths) {
        UICollectionViewLayoutAttributes *attributes = [self layoutAttributesForDecorationViewOfKind:CSSDecorationKind atIndexPath:indexPath];
        [decorationAttributes addObject:attributes];
      }
    
      return [allItems arrayByAddingObjectsFromArray:decorationAttributes];
    }
...
- (NSArray*)indexPathsOfSeparatorsInRect:(CGRect)rect {
  ...
  NSInteger lastCellIndexToShow = floorf((rect.origin.y + CGRectGetHeight(rect) - self.parallaxHeaderReferenceSize.height) / self.itemSize.height);
